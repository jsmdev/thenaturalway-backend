...
class SessionExerciseInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['exercise', 'order', 'sets_completed', 'repetitions', 'weight', 'rpe', 'rest_seconds', 'notes']
    readonly_fields = ['created_at', 'updated_at']


@admin.register(Session)
class SessionAdmin(admin.ModelAdmin):
    list_display = ['id', 'user', 'routine', 'date', 'duration_minutes', 'rpe', 'energy_level', 'created_at']
    ...
    fieldsets = (
        ...
        ('Métricas', {
            'fields': ('rpe', 'energy_level', 'sleep_hours')
        }),
        ('Notas', {
            'fields': ('notes',)
        }),
