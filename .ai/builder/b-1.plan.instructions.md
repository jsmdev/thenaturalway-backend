...
El proyecto puede implementar múltiples capas de presentación (API REST, Interfaz Web, Aplicación Móvil, etc.). **DEBES identificar qué capas de presentación se implementarán para esta funcionalidad**.

1. **Revisar la definición de la feature**:
   - Lee la sección "Capas de Presentación" de la definición de la feature
   - Identifica qué capas están documentadas (API REST, UI/UX, etc.)

2. **Pregunta sobre API REST** (si está documentada en la feature):
   - Si la feature documenta endpoints de API REST, pregunta: "¿Confirmas que se implementarán los endpoints de API REST documentados en la definición de la feature?"
   - Si la feature no documenta API REST pero el proyecto tiene API REST, pregunta: "¿Quieres que se implementen endpoints de API REST para esta funcionalidad además de la interfaz web?"
   - Incluye tareas para: serializadores, vistas API, rutas API, autenticación/permisos

3. **Pregunta sobre Interfaz de Usuario** (si está documentada en la feature):
   - Si la definición de la feature indica que no requiere UI (sección UI/UX con "No aplica"), confirma: "La definición de la feature indica que no requiere interfaz de usuario. ¿Confirmas que no necesitas capa de presentación web?"
   - Si la definición de la feature incluye UI/UX, pregunta: "¿Confirmas que se implementará la interfaz de usuario documentada en la definición de la feature?"
   - Si el usuario responde que SÍ quiere capa de presentación:
    ...
    - Tareas típicas: formularios, vistas web, templates HTML, rutas web, estilos CSS

4. **Combinaciones**:
   - **Solo API REST**: Tareas para serializadores, vistas API, rutas API
   - **Solo Interfaz Web**: Tareas para formularios, vistas web, templates, rutas web
   - **Ambas**: Tareas para ambas capas, asegurando que compartan la misma lógica de negocio (services/repositories)

Estas preguntas deben hacerse al inicio del proceso de planificación, antes de definir las tareas específicas.
