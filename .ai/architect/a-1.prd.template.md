...
- **Arquitectura**: {{ : monolítica | microservicios | orientada a servicios | otros }}
- **Patrón de comunicación**: {{ : síncrono | asíncrono | ambos }}
- **Autenticación**: {{ : ninguna | basada en tokens | basada en sesiones | otros }}
- **Autorización**: {{ : ninguna | basada en roles | basada en permisos | otros }}
- **Integraciones**: {{ : ninguna | APIs externas | servicios de terceros | otros }}
- **Monitorización**: {{ : operativa, de rendimiento, empresarial | ninguno | otros }}
- **Persistencia**: {{ : ninguna | sistema de archivos | base de datos relacional | base de datos no relacional | servicio remoto | múltiples }}

<!--
Un contenedor es un nivel de software físico o lógico, independiente y desplegable. Describe el propósito y responsabilidades del contenedor de forma agnóstica de tecnologías específicas.
Describe el arquetipo del contenedor (CLI, SPA, API, servicio de backend, etc.) y sus responsabilidades principales.
Ejemplo:
### C_1: Contenedor uno
- Una interfaz de línea de comandos que proporciona acceso a funcionalidades core de la aplicación.
-->
